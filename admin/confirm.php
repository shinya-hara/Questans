...
try {
  ...
  try {
    if ($_SESSION['update'] == 0) { // 新規作成時のみ確認
      // タイトルの取得
      $stmt = $dbh->prepare("select title from questionnaires where title = ? limit 1");
...
<html lang="ja">
  ...
  <body>
    ...
    <div class="container">
      ...
      <hr>
      <h3><?= h($_SESSION['title']) ?></h3>
      <div class="table-responsive">
        <table class="table text-center">
          <thead>
            <tr>
              <th class="td-num none">番号</th>
              <th>質問</th>
    ...
    $(function(){
      ...
      var callback = function(data) {
        if (data === "") {  // 作成成功時はリダイレクト
          window.location.href = "/admin/questionnaires_management.php";
        } else {
          $('#insert').prop('disabled', true);
        }
      }
      ...
      $('#insert').on('click', function() {
        if (/questionnaires_management\.php$/.test(document.referrer) || '<?=$_POST['update']?>' > 0) {
